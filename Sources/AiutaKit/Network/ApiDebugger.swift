...
@_spi(Aiuta) public extension ApiDebugger {
    ...
    func startOperation(title: String, subtitle: String?) async -> ApiDebuggerOperation? {
        await startOperation(id: nil, title: title, subtitle: subtitle)
    }

    func startOperation(title: String) async -> ApiDebuggerOperation? {
        await startOperation(id: nil, title: title, subtitle: nil)
    }
}
...
@_spi(Aiuta) public protocol ApiDebuggerRequest {
    ...
    var responseError: String? { get set }
}

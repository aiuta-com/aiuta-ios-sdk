...
@_spi(Aiuta) open class VRecycler<RecycleViewType, RecycleDataType>: Content<PlainView> where RecycleViewType: Recycle<RecycleDataType>, RecycleDataType: Equatable {
    ...
    public var data: DataProvider<RecycleDataType>? {
        didSet {
            ...
            guard oldValue?.items != data?.items else {
                if let data, data.canUpdate {
    ...
    public convenience init(_ builder: (_ it: VRecycler, _ ds: DesignSystem) -> Void) {
        self.init()
        builder(self, ds)
    }
}

private extension VRecycler {

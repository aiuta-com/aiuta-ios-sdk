...
class Label: Content<UILabel> {
    ...
    var isLineHeightMultipleEnabled = true {
        didSet { text = view.text }
    }

    var isAnimatedChanges = false
    var willLayoutParentOnChanges = true
    ...
    convenience init(_ builder: (_ it: Label, _ ds: DesignSystem) -> Void) {
        self.init()
    ...
    @discardableResult
    convenience init(view: UILabel, _ builder: (_ it: Label, _ ds: DesignSystem) -> Void) {
        self.init(view: view)
        view.isUserInteractionEnabled = false
        builder(self, ds)
    }

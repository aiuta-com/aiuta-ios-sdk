...
class HScroll: Content<UIScrollView> {
    var didScroll: Signal<(offset: CGFloat, delta: CGFloat)> { proxy.didScroll }
    ...
    convenience init(_ builder: (_ it: HScroll, _ ds: DesignSystem) -> Void) {
        self.init()
        builder(self, ds)
    }
    ...
    func scroll(to focus: ContentBase, animated: Bool = true) {

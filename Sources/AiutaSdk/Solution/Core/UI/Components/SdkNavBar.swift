...
final class NavBar: Plane {
    ...
    var backStyle: ButtonStyle? = .icon(.back) { didSet { updateStyle() } }
    var closeStyle: ButtonStyle? = .icon(.close) { didSet { updateStyle() } }
...
extension NavBar {
    ...
    enum ButtonStyle {
        case icon(SdkTheme.Navigation24)
        case label(String)
    }
}
...
private final class NavButton: PlainButton {
    var style: NavBar.ButtonStyle? {
        didSet {
            switch style {
                case let .icon(ref):
                    icon.image = ds.image.navigation(ref)
                    title.text = nil

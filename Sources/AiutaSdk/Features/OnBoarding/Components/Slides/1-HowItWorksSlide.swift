...
extension OnBoardingView.StickyScroll {
    ...
    final class InteractiveImage: Plane {
        let image1 = Image { it, ds in
            ...
            it.image = ds.image.onboarding(.how1L)
        }

        let image2 = Image { it, ds in
            it.isAutoSize = false
            it.contentMode = .scaleAspectFill
            it.image = ds.image.onboarding(.how2L)
        }

        let image3 = Image { it, ds in
            it.isAutoSize = false
            it.contentMode = .scaleAspectFill
            it.image = ds.image.onboarding(.how3L)
        }
    ...
    final class InteractiveNavigator: Shadow {
        let item1 = NavigationItem { it, ds in
            it.image.image = ds.image.onboarding(.how1S)
        }

        let item2 = NavigationItem { it, ds in
            it.image.image = ds.image.onboarding(.how2S)
        }

        let item3 = NavigationItem { it, ds in
            it.image.image = ds.image.onboarding(.how3S)
        }
        ...
        var scroll: OnBoardingView.StickyScroll? {
            firstParentOfType()
        }
    ...
    final class NavigationItem: PlainButton {
        ...
        override func updateLayout() {
            layout.make { make in
                ...
                make.radius = ds.dimensions.imageBoardingRadius // 9.6 + zoom * 3.6 // 9.6 ... 13.2
            }

...
final class AiutaSdkModelImpl: AiutaSdkModel {
    ...
    func startTryOn(_ input: Aiuta.Inputs) {
        Task {
            await startTryOn(input, uuid: sessionUuid)
        }
    }
    ...
    func addResults(_ generatedImages: [Aiuta.GeneratedImage], forInput input: Aiuta.Input, ofSku sku: Aiuta.SkuInfo) {
        let results: [Aiuta.SessionResult] = generatedImages.map { .output($0) }.reversed()
...
private extension AiutaSdkModelImpl {
    @MainActor func startTryOn(_ input: Aiuta.Inputs, uuid: UUID) async {
        ...
        switch input {
            case let .capturedImages(capturedImages):
                imagesToHistory = await capturedImages.concurrentCompactMap { [self] capturedImage in
                    ...
                    return await downsampleImageAndStart(capturedImage, sku: sku, ofInput: input, uuid: uuid)
                }
            ...
            case let .uploadedImages(uploadedImages):
                ...
                await uploadedImages.concurrentForEach { [self] uploadedImage in
                    ...
                    generationResults.append(.input(input, sku))
                    await startTryOn(uploadedImage, sku: sku, ofInput: input, uuid: uuid)
                }
        }
    }
    ...
    @MainActor func downsampleImageAndStart(_ image: UIImage, sku: Aiuta.SkuInfo, ofInput input: Aiuta.Input, uuid: UUID) async -> Aiuta.UploadedImage? {
...
@available(iOS 13.0, *)
private extension AiutaSdkModelImpl {
    @MainActor func startTryOn(_ image: Aiuta.UploadedImage, sku: Aiuta.SkuInfo, ofInput input: Aiuta.Input, uuid: UUID) async {
        ...
        repeat {
            ...
            if operation.status == .success {
                ...
                if sku == tryOnSku { changeState(.result, uuid: uuid) }
                return
            }
